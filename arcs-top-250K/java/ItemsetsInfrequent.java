...
public class ItemsetsInfrequent {
	...
	public static int populate(int start, int end, int fileNum, int extension, BufferedWriter out) {
		...
		for(int index=start; index<end; index++) {
			...
			if (extension == 8 && words[index].compareTo("standing/V") <= 0) {
				System.out.println("skipping "+words[index]);
				continue;
			}
			...
			for(int i=0; i<list.size(); i++) {
				...
				for(int j=i+1; j<list.size(); j++) {
					...
					if (val >= MIN_FREQ) pairCounts.put(key, val);
				}
			}
	...
	public static void main(String args[]) {
		...
		int list[] = {0,0,0,0,0,0,0,0,60,0}; // to pick up where was left off in case of interruption
		int fileNum = list[x];
